...
{
    public sealed class UserLoginAttemptEntityTypeConfiguration : IEntityTypeConfiguration<UserLoginAttempt>
    {
        public void Configure(EntityTypeBuilder<UserLoginAttempt> builder)
        {
            builder.HasKey(u => u.Id);
            builder.Property(u => u.UserLoginAttemptType).IsRequired();
            builder.Property(u => u.UserName).IsRequired().HasMaxLength(500);
            builder.Property(u => u.UserId).IsRequired(false);

            builder.HasOne(d => d.User)
                .WithMany(a => a.UserLoginAttempts)
